...
export const MaritimePage: React.FC = () => {
  ...
  const filteredVessels = useMemo(() => {
    return vessels.filter((v) => {
      ...
      if (filters.name && v.name && !v.name.toLowerCase().includes(filters.name.toLowerCase()))
        return false;
  ...
  return (
    <div className="absolute inset-0 bg-intel-bg overflow-hidden flex flex-col">
      <MaritimeToolbar totalCount={vessels.length} filteredCount={filteredVessels.length} />
      {/* <MaritimeLeftPanel /> */}
